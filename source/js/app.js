...
app.config(function($stateProvider, $urlRouterProvider, $locationProvider) {
	...
	$stateProvider
		...
		.state('events', {
			url: '/events',
			templateUrl: '/templates/_events.html',
		...
		.state('video', {
			url: '/video',
			templateUrl: '/templates/_video.html',
			pageTitle: 'Видео Evil Wheels',
			controller: 'videoCtrl'
		})
		.state('crew', {
			url: '/crew',
			templateUrl: '/templates/_crew.html',
			pageTitle: 'Состав Evil Wheels',
			controller: 'crewCtrl'
		})
		.state('contacts', {
			url: '/contacts',
			templateUrl: '/templates/_contacts.html',
		...
		.state('modal', {
			views: {
				'modal': {
					templateUrl: '/templates/_mainTrail.html'
				}
			},
			onEnter: function($state) {
				$(window).on('keyup', function(e) {
					if(e.keyCode == 27) {
						$(window).off('keyup');
						$state.go('map', {replace: true, reload: false});
					}
				});

				$(document).on('click', '#fadeComponent', function() {
					$state.go('map', {replace: true, reload: false});
				});

				$(document).on('click', '#closeComponent', function() {
					$state.go('map', {replace: true, reload: false});
				});
		...
		.state('modal.trail', {
			url: '/trails/:trailName',
			pageTitle: 'Трасса Evil Wheels',
			views: {
				'modal': {
					templateUrl: '/templates/_trail.html'
				}
			}
		});
...
appCtrl.controller('mainCtrl', ['$scope', '$http', function($scope, $http) {
	angular.element(document).ready(function() {
		var backgrounds = ["/data/events/cover/funnybunny.jpg", "/data/events/cover/funnybunnyvol2.jpg", "/data/events/cover/smoto2015.jpg", "/data/events/cover/smoto2016.jpg"];
...
appCtrl.controller('mapCtrl', ['$scope', '$state', function($scope, $state) {
	angular.element(document).ready(function() {
		var map = new L.map('trailMap', {scrollWheelZoom: false}).setView([43.20, 43.63], 8);
		...
		function displayModal(value) {
			$state.go('modal.trail', {trailName: value}, {replace: true, reload: false});
		}
	});
}]);

appCtrl.controller('trailCtrl', ['$scope', '$http', '$state', '$stateParams', function($scope, $http, $state, $stateParams) {
	$http.get('/data/trails/info/' + $stateParams.trailName.toLowerCase() + '.json').success(function(data) {
		$scope.trail = data;
	});

	angular.element(document).ready(function() {
		$http.get('/data/trails/info/' + $stateParams.trailName.toLowerCase() + '.json').success(function(data) {
			previewMap.setView([data.map.lat, data.map.lng], data.map.zoom);

			var eTrail = omnivore.kml('/data/trails/tracks/'+ $stateParams.trailName.toLowerCase() + '.kml').addTo(previewMap);

			eTrail.options.color = data.map.color;
		});

		$('button.primary').click(function() {
			$('button.primary').fadeOut(150);
			setTimeout(function() {
				$('#phoneservice').fadeIn(150);
			}, 200);
		});

		if($(window).width() >= 1200 && $(window).height() >= 768) {
			$("#viewComponent").css({"position":"absolute", "top": "50%", "left": "50%", "max-width": "1200px", "max-height": "620px", "margin-top": "-310px", "margin-left": "-600px"});
		}
...
appCtrl.controller('eventCtrl', ['$scope', '$http', '$state', '$stateParams', function($scope, $http, $state, $stateParams) {
	$http.get('/data/events/' + $stateParams.eventName.toLowerCase() + '.json').success(function(data) {
		$scope.event = data;
		...
		$.ajax({
			url: '/data/events/info/' + $stateParams.eventName.toLowerCase() + '.html',
			type: 'GET',
...
appCtrl.controller('locationCtrl', ['$scope', function($scope) {
	angular.element(document).ready(function() {
		...
		var locationAuto = omnivore.kml('/data/trails/tracks/location/auto.kml').addTo(locationMap);
		var locationPeople = omnivore.kml('/data/trails/tracks/location/people.kml').addTo(locationMap);
		var locationPeopleMarket = omnivore.kml('/data/trails/tracks/location/peoplemarket.kml').addTo(locationMap);
	...
	return {
		link: function() {
			var listener = function(event, toState) {
				$timeout(function() {
					$rootScope.title = (toState.pageTitle) ? toState.pageTitle : 'Команда Evil Wheels';
				});
			};
